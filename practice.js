...
var applyCoupon = function (groceries, coupon) {
  return _.map(groceries, function(grocery) {
    var numPrice = parseFloat(grocery.price.slice(1)) * 100;
    var discount = Math.round(numPrice * coupon);
    grocery.salePrice = '$' + (numPrice - discount) / 100;
    return grocery;
  });
};
...
var sumTotal = function (products) {
  return _.reduce(products, function(price, product) {
...
var countMessagesPerUser = function(tweets) {
  return _.reduce(tweets, function(memo, tweet) {
    if (!memo.hasOwnProperty(tweet.user)) {
      memo[tweet.user] = 1;
    } else {
      memo[tweet.user] ++;
    } return memo;
...
var movieNight = function (movies, timeLimit) {
  return _.reduce(movies, function(underTime, movie) {
    if (underTime) {
      return true;
    }
